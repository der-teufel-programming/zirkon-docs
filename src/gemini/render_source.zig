...
pub fn genSrc(
    ...
    const header = try std.fmt.allocPrint(allocator, "Source view for: {s}", .{file_path});
    defer allocator.free(header);
    try out.doc.addHeading(.h1, header);
    // try out.writeAll(
    ...
    defer arr.deinit();
    try reader.readAllArrayList(&arr, std.math.maxInt(usize));
    // defer allocator.free(source);
    const source = try arr.toOwnedSliceSentinel(0);
    defer allocator.free(source);
    out.initBlock();
    try tokenizeAndPrintRaw(out, source);
    const alt = try std.fmt.allocPrint(allocator, "rendered source code for {s}", .{file_path});
    defer allocator.free(alt);
    try out.endBlock(alt);
}
...
pub fn tokenizeAndPrintRaw(
    ...
    while (true) {
        ...
        if (mem.indexOf(u8, src[index..token.loc.start], "//")) |comment_start_off| {
            ...
            try out.rawWrite(src[index..comment_start]);
            try out.write(.{
                .source = src[comment_start..comment_end],
                .tag = .comment,
            });
            index = comment_end;
        ...
        try out.rawWrite(src[index..token.loc.start]);
        switch (token.tag) {
            .eof => break,
            ...
            => {
                ...
                try out.write(.{
                    .source = src[token.loc.start..token.loc.end],
                    .tag = .keyword,
                });
            },
            ...
            .multiline_string_literal_line => {
                if (src[token.loc.end - 1] == '\n') {
                    ...
                    try out.write(.{
                        .source = src[token.loc.start .. token.loc.end - 1],
                        .tag = .str,
                    });
                    // line_counter += 1;
